...
class DenoisingNet:
    ...
    def create_network(self, inp_w, inp_h):
        ...
        # Create network:
        # self._conv_layer_1 = self.convolutional_layer(self._X_norm, inp_channel = 1, op_channel = 1, strides = 2, name = "conv_1")
        # self._conv_layer_2 = self.convolutional_layer(self._conv_layer_1, inp_channel = 1, op_channel = 1, strides = 2, name = "conv_2")
        # self._conv_layer_3 = self.convolutional_layer(self._conv_layer_2, inp_channel = 2, op_channel = 2, strides = 2, name = "conv_3")
        # self._conv_layer_4 = self.convolutional_layer(self._conv_layer_3, inp_channel = 2, op_channel = 1, strides = 2, name = "conv_4")
        # self._conv_layer_5 = self.convolutional_layer(self._conv_layer_4, inp_channel = 16, op_channel = 32, strides = 2, name = "conv_5")



        #
        #
        # self._deconv_layer_1 = self.deconvolutional_layer(self._conv_layer_3, inp_shape = [None, 53, 66, 8], op_shape = [self._batch_size, 104, 134, 4],
        #                                                   strides = 2,
        #                                                   name = "deconv_1")
        # self._deconv_layer_2 = self.deconvolutional_layer(self._deconv_layer_1, inp_shape = [None, 104, 134, 4], op_shape = [self._batch_size, 209, 269, 2],
        #                                                   strides = 2,
        #                                                   name = "deconv_2")
        # self._deconv_layer_3 = self.deconvolutional_layer(self._deconv_layer_2, inp_shape = [None, 209, 269, 2], op_shape = [self._batch_size, inp_w, inp_h, 1],
        #                                                   strides = 2,
        #                                                   name = "deconv_3")

        # self._op = tf.reshape(tf.sigmoid(self._deconv_layer_3), shape = [-1, inp_w * inp_h])
        ...
        # self._conv_module_1 = self.convolutional_module_with_max_pool(x = self._X_norm, inp_channel = 1, op_channel = 1, name = "module_1", strides = 2)
        #
        # self._re = tf.reshape(self._conv_module_1, shape = [-1, 64])
        self._W_decode = tf.get_variable(name = "W_decode", shape = [100, inp_w * inp_h],
                                 initializer=tf.keras.initializers.he_normal())
        self._b_decode = tf.get_variable(name = "b_decode", shape = [inp_w * inp_h])
        self._X_reconstructed = tf.matmul(self._re_norm, self._W_decode) + self._b_decode
